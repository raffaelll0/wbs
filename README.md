# WB Servizi finanza agevolata

Sistema per la gestione di ingenti quantita di dati per la finanza agevolata di WB Servizi Globali
...
```bash
mkvirtualenv wb_servizi_ag -p python3.11
...
Create a database named `wb_servizi_ag`.

```
createdb wb_servizi_ag
```
...
```bash
celery -A wb_servizi_ag worker -l INFO
```

Or with celery beat (for scheduled tasks):

```bash
celery -A wb_servizi_ag worker -l INFO -B
```
